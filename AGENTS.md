...
- For API/worker integration changes: bring up infra, apply migrations, then `go run main.go` / `python worker.py` and exercise via `send_webhook.py` or `curl localhost:8080/health`.
