...
- `apps/worker` — Python worker. Consumes Redis queue `ai_review_jobs` (BRPOP), clones the target repo, runs tests/semgrep/npm audit, writes findings to Postgres. Shells out to `git`, `npm`, `pytest` — they must be on PATH. `semgrep` is installed via `requirement.txt` (pip) so it is available automatically after `pip install -r requirement.txt`. Serves Prometheus metrics on `:9090` (override with `METRICS_PORT`); metrics include `ai_review_latency_seconds` and `patch_verify_seconds`.
...
- `infra/`, `docker-compose.yml` — compose runs the full stack (Postgres + Redis + all four apps via per-app `Dockerfile`s); `docker compose up -d postgres redis` still brings up infra only for local dev. `infra/caddy/Caddyfile` configures the reverse proxy for production; `infra/{compose,docker}` are empty placeholders. Inside containers, services reach each other by name (`postgres`, `redis`, `api`, `ai-service`); the web container's rewrites use `API_INTERNAL_URL=http://api:8080` (defaults to localhost outside Docker). The web image needs `output: "standalone"` in `next.config.ts` — don't remove it.
